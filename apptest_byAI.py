...
class App:
    def __init__(self, root):
        self.root = root
        #enable_high_dpi_awareness(root=self.root, scaling=3)
        ...
        tk.Label(self.param_frame, text="Param Value/Tol").pack()

        self.param_val_entry = tk.Entry(self.param_frame, width=12)
        self.param_val_entry.insert(0, "2.5")
        self.param_val_entry.pack(side=tk.LEFT, padx=5)

        self.param_tol_entry = tk.Entry(self.param_frame, width=12)
        self.param_tol_entry.insert(0, "0.1")
        self.param_tol_entry.pack(side=tk.LEFT, padx=5)

        # Step size input
        tk.Label(self.control_frame, text="Step Size").pack(pady=5)
        self.step_entry = tk.Entry(self.control_frame, width=25)
        self.step_entry.insert(0, "0.1")
        self.step_entry.pack(pady=5)

        # Checkboxes for boolean options
        self.combine_run_var = tk.BooleanVar(value=True)
        tk.Checkbutton(self.control_frame, text="Combine Run", variable=self.combine_run_var).pack(pady=5)

        self.combine_fname_var = tk.BooleanVar(value=True)
        tk.Checkbutton(self.control_frame, text="Combine Filename", variable=self.combine_fname_var).pack(pady=5)

        self.combine_dc_var = tk.BooleanVar(value=True)
        tk.Checkbutton(self.control_frame, text="Combine DC", variable=self.combine_dc_var).pack(pady=5)
        ...
        self.plot_frame = tk.Frame(self.root) #<-------------------------------------- width=900
        self.plot_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

    def add_dropdown(self, label_text, var_name):
        """Add a dropdown menu to the control frame."""
